...
Please, please, **PLEASE!** Do **NOT** run code that is not hosted on a server. Do **NOT** run code from your browser using file://home/me/webVRworkshop/index.html because it will not work properly. It would be easier but based on our experience you WILL encounter problems very quickly and you won't know why. It will come from security issues, basically preventing cross-site scripting. Yes it might not be logical but if you want to avoid yourself an hour of headache just host your code on a proper HTTP server, even if a local one.
